...
std::set<Misspelling> spellcheck(const Corpus& source, const Dictionary& dictionary) {
  ...
  管道符在这里的作用是将左边的source传给右边的filter操作
  */
  namespace rv=std::ranges::views;
  
  auto view = source
    |rv::filter([&dictionary](Token token){
      return !dictionary.contains(token.content);//筛选出拼写有误的单词视图
    })
    |rv::transform([&dictionary](Token token){
      auto similar_words=dictionary|rv::filter([&token](std::string str){
        return 1==levenshtein(token.content,str);//对于拼写有误的单词，从dictionary里查找相似的单词视图
      });
      std::set<std::string> suggestions(similar_words.begin(),similar_words.end());//相似的单词组成集合
      return Misspelling{token,suggestions};//构造Misspelling
    })
